...
The standalone sample uses built-in demo content only: a still-life image and native SwiftUI document, diagram, report, and field-note previews. Its count selector shows 0, 1, 2, 3, and 6 examples. The empty state stays empty; one is a single calm preview, two use the preferred B·浮游 pair, three use an unequal layered arrangement, and four or more show the newest three sharp previews plus the actual fourth preview as a softly Gaussian-blurred full card behind them. That fourth card peeks below the group without a number or outline-only hint. The “播放连续加入（0→5）” control demonstrates five chronological insertions from empty through five items: each new preview becomes the top card, existing artwork identities smoothly move and re-layout, and the old fourth fades away when pushed beyond the visible sample. The 20-second timeline pauses between insertions for inspection. No item count appears inside the bubble. This remains built-in demo content and is not connected to the future collector or shelf.

The pearly shell evaluates curved sphere normals against a moving reflected studio environment. Its curved softbox ribbon changes arc, width, and return toward grazing angles; restrained thin-film color remains localized to the reflection, with a slightly brighter pointer-directed edge response. The separate light-sweep movie holds the content and shell breathing still while the reflection travels. At a 64-point shell size, the same count layouts remain within the shell and simplify their small document details. Arrival gathers the previews, holds for 0.9 seconds, then contracts them at the bubble's own center over 0.7 seconds.

## Build and preview on macOS

Run `scripts/build-and-capture.sh /path/to/output` on macOS with Xcode command line tools installed. The script compiles the app for Apple silicon and Intel, compiles the Metal shader, runs layout, motion, and render checks, and writes native PNG frames for sample counts and settled 0–5 history states at 226 and 64 points, synchronized count and insertion comparisons, center/rim lighting poses, patterned-background transmission, and arrival states. It also writes a 20-second 0→5 insertion movie, an isolated light sweep, an arrival movie, and packages `BubbleMaterialLab.app` in the output folder.

Open `BubbleMaterialLab.app` for the live pointer and motion interaction. In the preview window, use the count selector to choose a built-in sample state, play the arrival pass, or play the complete 0→5 file-insertion sequence. The capture harness drives the same SwiftUI scene and insertion timeline with fixed inputs so its output is repeatable.
