...
The standalone sample uses built-in demo content only: a still-life image and native SwiftUI document, diagram, report, and field-note previews. Its count selector and synchronized comparison show 0, 1, 2, 3, and 6 examples. The empty state stays empty; one is a single calm preview, two use the preferred B·浮游 pair, three use an unequal layered arrangement, and four or more show the newest three sharp previews plus the actual fourth preview as a softly Gaussian-blurred full card behind them. That fourth card peeks below the group without a number or outline-only hint. The “播放新文件加入” control demonstrates chronological 3→4→5 insertion: stable artwork identities move back and re-layout as each new preview becomes the top card, while the old fourth fades away when it is pushed beyond the visible sample. The timeline holds each settled arrangement for inspection. No item count appears inside the bubble. This remains built-in demo content and is not connected to the future collector or shelf.
...
Run `scripts/build-and-capture.sh /path/to/output` on macOS with Xcode command line tools installed. The script compiles the app for Apple silicon and Intel, compiles the Metal shader, runs layout, motion, and render checks, and writes native PNG frames for all five counts at 226 and 64 points, a synchronized count comparison, a static 3/4/5 insertion comparison, settled and transition history states, center/rim lighting poses, patterned-background transmission, and arrival states. It also writes a 12-second 3→4→5 insertion movie, an isolated light sweep, an arrival movie, and packages `BubbleMaterialLab.app` in the output folder.

Open `BubbleMaterialLab.app` for the live pointer and motion interaction. In the preview window, use the count selector to choose a built-in sample state, play the arrival pass, or play the file-insertion sequence. The capture harness drives the same SwiftUI scene and insertion timeline with fixed inputs so its output is repeatable.
