...
5. 切换 Claude / Codex，查看剩余百分比、重置日期时间、上次更新及下次可查询时间。
   重置时间使用 Mac 当前时区。每个窗口右边的「固定」菜单可选择左侧、右侧或取消。
   左右各一个窗口；同一窗口换边会从原侧移走，选择新窗口会替换该侧旧选择。
6. 收起时，两侧显示细圆环与剩余百分比，点击进入对应服务。默认音乐播放时让位，
   暂停后恢复；关闭「音乐播放时暂让位」则额度优先，音乐仍可在 Home 页控制。
   音量、亮度、电池、切歌等短暂提示始终优先，提示结束自动恢复，不同时叠画。
   缺失数据使用「—」；失败后的旧数据使用灰环并标「旧」。未固定的一侧留空以保持
   摄像头区域居中。隐藏刘海/高度为零时不显示固定额度。
7. AI 页中滚轮和触控板只滚动内容，不触发原版上滑收起或整体缩放；移开鼠标仍按
   原版设置收起。Home/Shelf 的开关手势保持原有设置。

圆环代表服务端剩余额度，不是 token 或费用。Codex 的订阅窗口无法分解到单个模型；
Claude 只有接口实际提供模型窗口时才显示该模型。两侧选择及音乐优先设置保存在本机。
