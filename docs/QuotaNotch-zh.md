...
6. 收起时，只有额度则左边是服务图形，右边是细圆环；只有音乐则沿用
   ...
   音量、亮度、静音及切歌信息在当前内容下方追加一行，不改变主内容模式或宽度；
   主内容继续实时更新。控制条停止操作约 1.5 秒后收回，连续操作会延长显示时间。
   电池等替换式通知保留独立优先级。空闲表情与会替换主内容的内嵌 HUD 样式已移除。
...
`PrimaryNotchLayout` 只测量第一个子视图来决定宽度，再把附加行限制到该宽度；
不依赖音乐/额度的具体组合、图标数量或固定宽度。事件通过 `NotchEventPlacement`
明确声明追加还是替换。`Accessory-Layout.png` 展示各模式追加音量条的效果；
渲染检查覆盖 6 种宽度、4 种内容和 1–2 行附加内容，检查上方像素及宽度保持一致。
