...
struct AgentSession: Identifiable, Equatable, Codable {
    ...
    var displayTitle: String { title.isEmpty ? "Codex · " + String(id.prefix(8)) : title }
    var eventID: String { id + ":" + turnID + ":" + state.rawValue }
...
enum AgentEventParser {
    ...
    static func apply(_ data: Data, to session: inout AgentSession) {
        ...
        if kind == "event_msg" {
            ...
            case "agent_message", "agent_reasoning", "exec_command_begin", "exec_command_end", "item_completed":
                break
            default: return
            }
    ...
    static func applyHook(_ object: [String: Any], to session: inout AgentSession) {
        ...
        case "UserPromptSubmit":
            ...
            session.state = .running; session.startedAt = at; session.finishedAt = nil
        case "PermissionRequest": session.state = .waiting
