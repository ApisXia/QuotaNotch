...
enum BubbleNotchLayout {
    ...
    /// The notch mark is an explicit presentation preference. Saved items and
    /// the manual holder must not make a new resident accessory appear by
    /// themselves.
    static func shouldShowPinnedGlyph(isPinnedToNotch: Bool) -> Bool {
        isPinnedToNotch
    }

    /// Legacy helper retained for preview fixtures that model the former
    /// receive-mode behavior. Production ContentView uses the explicit pin
    /// preference above.
...
/// The result of routing one AppKit scroll sample through a full notch wing.
/// `vertical` deliberately passes the event to the child receiver so the
/// Shelf can still toggle receiving; `consume` keeps an unclaimed horizontal
/// gesture away from that receiver while its direction accumulates.
